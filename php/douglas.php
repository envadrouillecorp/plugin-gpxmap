...
class Douglas {
   public function simplify_RDP($vertices, $tolerance) {
      ...
      if (is_array($vertices[0][0])) {
         $multi = array();
         foreach ($vertices as $subvertices) $multi[] = Douglas::simplify_RDP($subvertices,$tolerance);
         return $multi;
      }
      $tolerance2 = $tolerance * $tolerance;
      // okay, so this is a single linestring and we simplify it individually
      return Douglas::_segment_RDP($vertices,$tolerance2);
   }

   public static function _segment_RDP($segment, $tolerance_squared) {
      ...
      $segs1 = Douglas::_segment_RDP($slice1, $tolerance_squared);
      $segs2 = Douglas::_segment_RDP($slice2, $tolerance_squared);
